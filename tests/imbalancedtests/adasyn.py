...
for tab in range(5):
    ...
    table.add_row(('', "Bag TREE", "Bag TREE ADASYN", "AB TREE", "AB TREE ADASYN", "Stacking", "Stacking ADASYN"))
    table.add_hline()
    tables.append(table)
...
for data in dataset:
    ...
    for clf in clfs:
        ...
        # ADASYN
        adasyn_ = ADASYN(random_state=random_st)
        ...
        for iteration in range(iterations):
            ...
            for train_index, test_index in skf.split(db.data, db.target):
                data_re, tar_re = adasyn_.fit_sample(db.data[train_index], db.target[train_index])
                clf_ = clone(clf)
...
doc = Document("oversampling_ADASYN")
