...
for tab in range(5):
    ...
    table.add_row(('', "Bag TREE", "Bag TREE SMOTEENN", "AB TREE", "AB TREE SMOTEENN", "Stacking", "Stacking SMOTEENN"))
    table.add_hline()
    tables.append(table)
...
for data in dataset:
    ...
    for clf in clfs:
        ...
        # SMOTEENN
        sme = SMOTEENN(random_state=random_st)
        ...
        for iteration in range(iterations):
            ...
            for train_index, test_index in skf.split(db.data, db.target):
                data_re, tar_re = sme.fit_sample(db.data[train_index], db.target[train_index])
                clf_ = clone(clf)
