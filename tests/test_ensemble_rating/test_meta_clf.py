...
for tab in range(5):
    ...
    table.add_hline()
    table.add_row(('', "BKNN", "BTREE", "BNB", "ATREE", "ANB", "KNN", "TREE", "NB", "ESR", "META"))
    table.add_hline()
    tables.append(table)
...
clfs = [BaggingClassifier(KNeighborsClassifier(), n_estimators=100, max_samples=0.9),
        BaggingClassifier(tree.DecisionTreeClassifier(), n_estimators=100, max_samples=0.9),
        BaggingClassifier(GaussianNB(), n_estimators=100, max_samples=0.9)]
clfs.extend([AdaBoostClassifier(tree.DecisionTreeClassifier(max_depth=3), n_estimators=50),
             AdaBoostClassifier(GaussianNB(), n_estimators=50)])

clfs.extend([clf1, clf2, clf3, prec_clf1, m_clf])
