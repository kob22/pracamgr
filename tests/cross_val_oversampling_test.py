...
def cross_val_oversampling_wrong(data, target):
    ...
    for clf, row in zip(clfs_normal, rows_normal):
        ...
        print(row_temp)
        row_temp.append(row_temp[2] - row_temp[6])
        row.extend(row_temp)
...
tables2 = []
for tab in (0, 2, 4, 6):
    table = Tabular('|c|c|c|c|c|c|c|c|c|c|')
    table.add_hline()
    table.add_row('', MultiColumn(4, align='|c|', data=names2[tab]), MultiColumn(4, align='|c|', data=names2[tab + 1]),
                  '')
    table.add_hline(start=2)
    table.add_row(('', "Sp", "F1", "G", "AUC", "Sp", "F1", "G_t", "AUC", 'G-G_t'))
    table.add_hline()
...
for tab in (0, 2, 4, 6):
    table = Tabular('|c|c|c|c|c|c|c|c|c|c|')
    table.add_hline()
    table.add_row('', MultiColumn(4, align='|c|', data=names3[tab]), MultiColumn(4, align='|c|', data=names3[tab + 1]),
                  '')
    table.add_hline(start=2)
    table.add_row(('', "Sp", "F1", "G", "AUC", "Sp", "F1", "G_t", "AUC", 'G-G_t'))
    table.add_hline()
